    ...
    RecyclerView.Adapter<ProfileListAdapter.ProfileViewHolder>()
{
    ...
    override fun onBindViewHolder(holder: ProfileViewHolder, position: Int)
    {
        ...
        holder.content.itemProfileCapacityTv.text =
            profile.accConfig.configCapacity.toString(mContext)
        ...
        holder.content.itemProfileSwitchDataTv.text =
            profile.accConfig.configChargeSwitch ?: mContext.getString(R.string.automatic)
        ...
        holder.content.itemProfileChargingVoltageLl.isVisible =
            profile.pEnables.eVoltage || profile.pEnables.eCurrMax

        holder.content.itemProfileChargingVoltageTv.text =
            profile.accConfig.configVoltage.toString(mContext)
        holder.content.itemProfileCurrentMaxTv.text =
            mContext.getString(R.string.current_max) + " " + profile.accConfig.configCurrMax.toString()

        if ((profile.accConfig.configVoltage.controlFile == null || profile.accConfig.configVoltage.max == null) && profile.accConfig.configCurrMax == null)
        else
        {
            holder.content.itemProfileChargingVoltageTv.isGone =
                profile.accConfig.configVoltage.controlFile == null && profile.accConfig.configVoltage.max == null
            holder.content.itemProfileCurrentMaxTv.isGone = profile.accConfig.configCurrMax == null
        }

        //----------------------------------------------------

        holder.content.itemProfileTemperatureLl.isVisible = profile.pEnables.eTemperature
        holder.content.itemProfileTemperatureTv.text =
            profile.accConfig.configTemperature.toString(mContext)
        ...
        holder.content.itemProfileOnBootTv.text = if (profile.accConfig.configOnBoot == null) "-"
        else profile.accConfig.configOnBoot

        holder.content.itemProfileOnPlugLl.isVisible = profile.pEnables.eRunOnPlug
        holder.content.itemProfileOnPlugTv.text = if (profile.accConfig.configOnPlug == null) "-"
        else profile.accConfig.getOnPlug(mContext)

        holder.content.itemProfilePrioritizeBatteryIdleTv.isVisible =
            profile.accConfig.prioritizeBatteryIdleMode
        holder.content.itemProfileResetBsOnPauseTv.isVisible =
            profile.accConfig.configResetBsOnPause
        holder.content.itemProfileResettUnpluggedTv.isVisible =
            profile.accConfig.configResetUnplugged
