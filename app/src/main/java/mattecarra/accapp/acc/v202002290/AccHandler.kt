...
open class AccHandler: AccInterface {
    ...
    override suspend fun isAccdRunning(): Boolean = withContext(Dispatchers.IO) {
        Shell.su("/sbin/acca -D").exec().out.find { it.contains(" is running") } != null
    }
    ...
    override fun getUpdateResetUnpluggedCommand(resetUnplugged: Boolean) = "/sbin/acca -s reset_batt_stats_on_unplug=$resetUnplugged"

    override fun getUpdateResetOnPauseCommand(resetOnPause: Boolean) = "/sbin/acca -s reset_batt_stats_on_pause=$resetOnPause"

    override fun getUpdateAccCoolDownCommand(charge: Int?, pause: Int?): String = "/sbin/acca -s cooldown_charge=${charge?.toString().orEmpty()} cooldown_pause=${pause?.toString().orEmpty()}"

    override fun getUpdateAccCapacityCommand(shutdown: Int, coolDown: Int, resume: Int, pause: Int): String = "/sbin/acca -s shutdown_capacity=$shutdown cooldown_capacity=$coolDown resume_capacity=$resume pause_capacity=$pause"

    override fun getUpdateAccTemperatureCommand(coolDownTemperature: Int, temperatureMax: Int, wait: Int): String = "/sbin/acca -s cooldown_temp=${coolDownTemperature} max_temp=${temperatureMax} max_temp_pause=$wait"

    override fun getUpdateAccVoltControlCommand(voltFile: String?, voltMax: Int?): String = "/sbin/acca --set max_charging_voltage=${voltMax?.toString().orEmpty()}"

    override fun getUpdateAccCurrentMaxCommand(currMax: Int?): String = "/sbin/acca --set max_charging_current=${currMax?.toString().orEmpty()}"

    override fun getUpdateAccOnBootExitCommand(enabled: Boolean): String = "" //Not supported

    override fun getUpdateAccOnBootCommand(command: String?): String = "/sbin/acca -s \"apply_on_boot=${command.orEmpty()}\""
